...
class GaussianDiffusion(nn.Module):
    ...
    def get_betas(self):
        ...
        if self.noise_schedule == "linear" or self.noise_schedule == "linear-var":
            start = self.noise_scale * self.noise_min#0.1 *0.0001
            end = self.noise_scale * self.noise_max#0.1*0.02:
            self.noise_schedule = "linear"
            if self.noise_schedule == "linear":
                # return np.linspace(start, end, self.steps, dtype=np.float64)#原先的
                ##参考 latent-diffusion
                return np.linspace(start**0.5,end**0.5, self.steps, dtype=np.float64)**2
    ...
    def calculate_for_diffusion(self):
        alphas = 1.0 - self.betas#5个
        self.alphas_cumprod = th.cumprod(alphas, axis=0).to(self.device)#累积乘法
    ...
    def p_sample(self, model, iftraining,latemb, steps,sampling_noise=False):
        # assert steps <= self.steps, "Too much steps in inference."
        x_start ,conemb =  latemb
        # steps = 0
        if steps == 0:
            x_t = x_start
        else:
            t = th.tensor([steps - 1] * x_start.shape[0]).to(x_start.device)#每个用户都是4
            x_t = self.q_sample(x_start, t)#前向过程 400 300
        # if  iftraining==False:
        #     x_t = x_start
        ...
        if self.noise_scale == 0.:
            for i in indices:
                ...
                x_t = model(x_t,conemb, t)
            return x_t
        #一步一步往前推导
        for i in indices:# indices [4, 3, 2, 1, 0] 逆向着来
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)#7433个4
            out = self.p_mean_variance(model, x_t,conemb, t,iftraining)#好像用之前训练好的MLP网络来一步一步往回倒
            if sampling_noise:
    ...
    def training_losses(self,model,iftraining,embed,reweight=False):
        x_start,attembed = embed
        diff = 0
        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, device, diff,'importance')
        ...
        if self.noise_scale != 0.:
            ...
            x_t = x_start
        terms = {} 
        model_output = model(x_t,attembed, ts,iftraining)
        target = {
    ...
    def sample_timesteps(self, batch_size, device, diff,method='uniform', uniform_prob=0.001):
        method =  'uniform'
        if method == 'importance': 
            Lt_sqrt = th.sqrt(diff)
    ...
    def q_sample(self, x_start, t, noise=None):
        if noise is None:
            noise = th.randn_like(x_start).cuda()
            
        assert noise.shape == x_start.shape
        return (#self.sqrt_alphas_cumprod 是每个step的值，--》将其与 x_start的形状对齐
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        posterior_mean = (
              self._extract_into_tensor(self.posterior_mean_coef1, t, x_t.shape) * x_start# 第三个
            + self._extract_into_tensor(self.posterior_mean_coef2, t, x_t.shape) * x_t#第四个 t=4
        )
    ...
    def p_mean_variance(self, model, x,conemb, t,iftraining):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B, )
        model_output = model(x, conemb,t,iftraining)
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        ...
        z = th.randn_like(x_t)*(((t>0)*1).unsqueeze(1))
        return (
            self._extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t
            - self._extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape) * eps #+ th.sqrt(self._extract_into_tensor(self.betas, t, x_t.shape))*z
        )
    ...
    def _extract_into_tensor(self, arr, timesteps, broadcast_shape):
        ...
        :param timesteps: a tensor of indices into the array to extract.#400
        :param broadcast_shape: a larger shape of K dimensions with the batch
                                dimension equal to the length of timesteps.
        :return: a tensor of shape [batch_size, 1, ...] where the shape has K dims.
        """
        # res = th.from_numpy(arr).to(device=timesteps.device)[timesteps].float()
        arr = arr.to(timesteps.device)#1*5# 5个不一样的值
        res = arr[timesteps].float()#400
        while len(res.shape) < len(broadcast_shape):
            res = res[..., None]#400 1
        return res.expand(broadcast_shape)#400 300
